...
function gotoSlide(destination) {
    ...
    slideContainer.animate({left: destLeft}, 400);

    // Fade out current slide
    $(currentFormSlide).animate({visibility: 0}, 400);

    // Fade in destination slide
    $(currentFormSlide).animate({visibility: 1}, 400);
    ...
    if(destination == totalSlides) {
        nextBtn.fadeOut();
        submitBtn.fadeIn();
        feedForwardText.fadeIn();
    } else {
        nextBtn.fadeIn();
        submitBtn.fadeOut();
        feedForwardText.fadeOut();
    }
}
